...
import { Drumstick, RefreshCw, Blend, Flower2, Sparkles } from 'lucide-react';
...
const Tecnicas = () => {
  return (
    <Layout>
      {/* Hero Section */}
      <section className="from-card to-background relative overflow-hidden bg-gradient-to-b py-32">
        {/* Decorative elements */}
        <div className="absolute inset-0 opacity-10">
          <div className="bg-gold absolute left-1/4 top-20 size-96 rounded-full blur-3xl" />
          <div className="bg-copper absolute bottom-0 right-1/4 size-64 rounded-full blur-3xl" />
        </div>
        ...
        <div className="container relative z-10 mx-auto px-4">
          <div className="animate-slide-up mx-auto max-w-3xl text-center">
            ...
            <h1 className="font-display text-foreground mb-6 text-4xl font-bold leading-tight md:text-6xl">
              Técnicas de <span className="text-gradient-gold">Bartender</span>
            </h1>
            <p className="text-muted-foreground text-lg leading-relaxed md:text-xl">
              Domine as técnicas fundamentais que todo bartender profissional utiliza para criar drinks perfeitos.
            </p>
          </div>
        </div>
      </section>
                  ...
                  <div className="border-border mt-6 border-t pt-6">
                    ...
                    <ul className="space-y-3">
                      {technique.tips.map((tip, tipIndex) => (
                        <li
                          key={tipIndex}
                          className="text-muted-foreground group-hover:text-foreground/80 flex items-start gap-3 text-sm transition-colors"
                        >
